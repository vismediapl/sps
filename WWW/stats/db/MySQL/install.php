...
if (!mysql_select_db ($_POST['DBName'])) $Errors['DBStructure'] = 1;
...
if ($_POST['Action'] == 'upgrade' && $CurrentDB != 'TXT') {
   $Input = array ();
   $Result = mysql_query ('SELECT * FROM `'.$_POST['DBPrefix'].'oses`');
...
';
   for ($i = 0, $c = count ($OSes); $i < $c; $i++) $SQL.= 'INSERT INTO `'.$_POST['DBPrefix'].'oses` VALUES(\''.$OSes[$i][0].'\', \''.mysql_escape_string ($OSes[$i][1]).'\', '.mysql_escape_string ($OSes[$i][2]).', \''.$OSes[$i][3].'\');
';
...
foreach ($Array as $Group => $Value) {
        foreach ($Value as $SGroup => $Option) {
                if (is_array (reset ($Option))) {
                   foreach ($Option as $Field => $SOption) $SQL.= 'INSERT INTO `'.$_POST['DBPrefix'].'configuration` VALUES(\''.$SGroup.'|'.$Field.'\', \''.mysql_escape_string ($SOption[0]).'\', '.(int) ($Group != 'Stats').');
';
                   }
                else $SQL.= 'INSERT INTO `'.$_POST['DBPrefix'].'configuration` VALUES(\''.$SGroup.'\', \''.mysql_escape_string ($Option[0]).'\', '.(int) ($Group != 'Stats').');
';
                }
        }
if ($_POST['Action'] == 'upgrade') {
   if (isset ($_POST['convertlogs']) && isset ($LogConverted)) {
      for ($i = 0, $c = count ($LogConverted); $i < $c; $i++) $SQL.= 'INSERT INTO `'.$_POST['DBPrefix'].'logs` VALUES('.$LogConverted[$i][0].', '.$LogConverted[$i][1].', \''.mysql_escape_string ($LogConverted[$i][2]).'\');';
      }
   if ($CurrentDB == 'TXT') {
      for ($i = 0, $c = count ($Files); $i < $c; $i++) {
          if ($Files[$i] == 'vbrowsers') $Files[$i] = 'browsers';
          for ($j = 0, $l = count ($Output[$Files[$i]]); $j < $l; $j++) $SQL.= 'INSERT INTO `'.$_POST['DBPrefix'].$Files[$i].'` VALUES(1, \''.mysql_escape_string ($Output[$Files[$i]][$j][0]).'\', '.$Output[$Files[$i]][$j][1].(in_array ($Files[$i], array ('browsers', 'oses', 'sites'))?', \''.mysql_escape_string ($Output[$Files[$i]][$j][2]).'\'':'').');
';
          }
...
if (!isset ($_POST['onlygeneratesql'])) {
   $Array = explode (';
', $SQL);
   for ($i = 0, $c = count ($Array); $i < $c; $i++) if ($Array[$i]) mysql_query ($Array[$i]) or $Errors['DBStructure'] = 1;
   }
?>