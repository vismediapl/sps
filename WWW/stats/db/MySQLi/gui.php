...
function data ($Table, $HowMany, $Year, $Month) {
         ...
         $Result = mysqli_query ($this->DB, 'SELECT '.(($Table == 'vbrowsers' || $Table == 'voses')?'CONCAT(`name`," ",`version`) AS `ver`, SUM(`num`) AS `num` FROM `'.eDBPREFIX.substr ($Table, 1).'`':'`name`, SUM(`num`) AS `num`'.(($Table == 'sites')?', `address`':'').' FROM `'.eDBPREFIX.$Table.'`').$Query.' GROUP BY `'.(($Table == 'vbrowsers' || $Table == 'voses')?'ver':(($Table == 'sites')?'address':'name')).'` ORDER BY `num` DESC LIMIT '.$HowMany);
         while ($Row = mysqli_fetch_row ($Result)) $Data[0][$Row[($Table == 'sites')?2:0]] = (($Table == 'sites')?array ($Row[1], ($Row[0]?$Row[0]:$Row[2])):$Row[1]);
         $Data[1] = $this->query ('SELECT SUM(`num`) FROM `'.eDBPREFIX.(($Table == 'vbrowsers' || $Table == 'voses')?substr ($Table, 1):$Table).'`'.$Query, 1);
         $Data[2] = mysqli_num_rows (mysqli_query ($this->DB, 'SELECT COUNT(`num`) FROM `'.eDBPREFIX.(($Table == 'vbrowsers' || $Table == 'voses')?substr ($Table, 1):$Table).'`'.$Query.' GROUP BY `name`'.(($Table == 'vbrowsers' || $Table == 'voses')?', `version`':(($Table  == 'sites')?', `address`':''))));
         return ($Data);
         }
function time () {
         $this->clean ();
	...
	array ('`day`, `uni`, `all` FROM `'.eDBPREFIX.'daysofweekpopularity`', 7)
         ...
         for ($i = 0, $c = count ($Array); $i < $c; $i++) {
             $Result = mysqli_query ($this->DB, 'SELECT '.$Array[$i][0]);
             $TData = array ();
...
function detailed ($Robots, $Page) {
         ...
         $Result = mysqli_query ($this->DB, 'SELECT `v`.*, MIN(`d`.`time`), MAX(`d`.`time`) AS `vtime`, COUNT(`d`.`time`) FROM `'.eDBPREFIX.'visitors` `v`, `'.eDBPREFIX.'details` `d` WHERE `d`.`uid` = `v`.`uid`'.($Robots?'':' AND `v`.`robot` = "0"').' GROUP BY `uid` ORDER BY `vtime` DESC LIMIT '.($Detailed['amount'] * ($Page - 1)).', '.$Detailed['amount']);
         while ($Row = mysqli_fetch_row ($Result)) $Data[0][] = $Row;
         return ($Data);
         }
function details ($ID, $Page) {
         ...
         $Result = mysqli_query ($this->DB, 'SELECT `time`, `sid` FROM `'.eDBPREFIX.'details` WHERE `uid` = '.(int) $ID.' ORDER BY `time` DESC LIMIT '.($Amount * ($Page - 1)).', '.$Amount);
         $Sites = array ();
         while ($Row = mysqli_fetch_row ($Result)) $Sites[] = array ($Row[0], $Row[1], $this->query ('SELECT `name` FROM `'.eDBPREFIX.'sites` WHERE `address` = \''.mysqli_escape_string ($this->DB, $Sites[$i][1]).'\' ORDER BY `time` DESC LIMIT 1', 1));
...
function archive ($Year, $Month) {
         ...
         $Result = mysqli_query ($this->DB, 'SELECT '.($Month?'DAYOFMONTH':'MONTH').'(`date`)'.($Month?'':' as `month`').', '.($Month?'`uni`, `all`':'SUM(`uni`), SUM(`all`)').' FROM `'.eDBPREFIX.'archive` WHERE YEAR(`date`) = '.$Year.($Month?' AND MONTH(`date`) = '.$Month:' GROUP BY `month`'));
         while ($Row = mysqli_fetch_row ($Result)) {
...
function ignored ($Page, $Amount) {
         ...
         $Result = mysqli_query ($this->DB, 'SELECT * FROM `'.eDBPREFIX.'ignored` ORDER BY "lastall" DESC '.($Amount?'LIMIT '.$From.', '.$Amount:''));
         while ($Row = mysqli_fetch_row ($Result)) $Array[] = $Row;
         return (array ($Array, $HowMany, $Page));
         }
function logs ($Page, $Amount, $Search = 0) {
         if ($Search) $Where = (($_POST['from'] && $_POST['to'])?' WHERE `time` >= '.strtotime ($_POST['from']).' AND `time` <= '.strtotime ($_POST['to']):'').(isset ($_POST['filter'])?' AND (`log` = '.implode (' OR `log` = ', $_POST['filter']).')':'').($_POST['search']?' AND (`'.implode ('` LIKE "%'.mysqli_escape_string ($this->DB, $_POST['search']).'%" OR `', array ('log', 'time', 'info')).'` LIKE "%'.mysqli_escape_string ($this->DB, $_POST['search']).'%")':'');
         else $Where = '';
         ...
         $Array = array ();
         if ($RAmount) {
            $Result = mysqli_query ($this->DB, 'SELECT * FROM `'.eDBPREFIX.'logs`'.$Where.' ORDER BY `time`'.($Amount?' LIMIT '.$From.', '.$Amount:''));
            while ($Row = mysqli_fetch_row ($Result)) $Array[] = $Row;
            }
...
function delete_row ($Table, $Row) {
         if (eULEVEL == 2) {
            mysqli_query ($this->DB, 'DELETE FROM `'.eDBPREFIX.$Table.'` WHERE `name` = "'.$Row.'"');
            return (mysqli_affected_rows ($this->DB)?1:0);
            }
         }
function reset () {
         if (isset ($_POST['RDetailed'])) {
            mysqli_query ($this->DB, 'TRUNCATE TABLE `'.eDBPREFIX.'visitors`');
            mysqli_query ($this->DB, 'TRUNCATE TABLE `'.eDBPREFIX.'details`');
            e_log (30, 1);
            return (0);
            }
         global $DBTables;
         if (isset ($_POST['RAll'])) {
            for ($i = 0, $c = count ($DBTables); $i < $c; $i++) if (!in_array ($DBTables[$i], array ('configuration', 'logs'))) mysqli_query ($this->DB, in_array ($DBTables[$i], array ('hourspopularity', 'daysofweekpopularity'))?'UPDATE `'.eDBPREFIX.$DBTables[$i].'` SET `uni` = 0, `all` = 0':'TRUNCATE TABLE `'.eDBPREFIX.$DBTables[$i].'`');
            $this->config_set (array ('LastReset' => time ()), 0);
            e_log (31, 1);
            return (0);
            }
         if (isset ($_POST['RSTable']) && in_array ($_POST['RTable'], $DBTables)) {
            mysqli_query ($this->DB, 'TRUNCATE TABLE `'.eDBPREFIX.$_POST['RTable'].'`');
...
', file_get_contents ('data/backups/'.$BID.'.sql.bak'));
         for ($i = 0, $c = count ($Array); $i < $c; $i++) if ($Array[$i]) mysqli_query ($this->DB, $Array[$i]) or $Error = 1;
         e_log (($Error?25:24), !$Error, 'ID: '.$BID);
         }
function db_size () {
         global $DBTables, $DBName;
         $Array = array ('all' => 0, 'visitors' => 0, 'details' => 0);
         $Result = mysqli_query ($this->DB, 'SHOW TABLE STATUS FROM `'.$DBName.'` LIKE "'.eDBPREFIX.'%"');
         while ($Row = mysqli_fetch_array ($Result)) if (in_array ($TName = substr ($Row['Name'], strlen (eDBPREFIX)), $DBTables)) $Array['all'] += ($Array[$TName] = $Row['Data_length']);
...
function clean () {
         ...
         global $Detailed;
         mysqli_query ($this->DB, 'DELETE FROM `'.eDBPREFIX.'hours` WHERE `time` < "'.date ('Y-m-d H:00:00', strtotime ('last day')).'"');
         if (!$Detailed['keepalldata']) {
            ...
            $Result = mysqli_query ($this->DB, 'SELECT `uid`, MAX(`time`), MIN(`time`) FROM `'.eDBPREFIX.'details` GROUP BY `uid`');
            while ($Row = mysqli_fetch_row ($Result)) {
                  if ($Row[1] <= $Time && (time () - $Row[2]) > $Detailed['period'] && (time () - $Row[2]) > $GLOBALS['Time']) $UIDs[] = $Row[0];
                  }
            if ($UIDs) {
               mysqli_query ($this->DB, 'DELETE FROM `'.eDBPREFIX.'details` WHERE `uid` = '.implode (' OR `uid` = ', $UIDs));
               mysqli_query ($this->DB, 'DELETE FROM `'.eDBPREFIX.'visitors` WHERE `uid` = '.implode (' OR `uid` = ', $UIDs));
               }
            }
